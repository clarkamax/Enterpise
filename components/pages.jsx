      ...
      <section className="hero">
        <div className="container">
          ...
          <div className="hero-photo">
            {photo.src && <img src={photo.src} alt={`${photo.cat} — ${photo.title}`} loading="lazy" decoding="async" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />}
                ...
                <>
                  <div className="field-row">
                    <div className={`field ${errors.name ? "err" : ""}`}>
                      ...
                      <input id="c-name" name="name" autoComplete="name" aria-invalid={!!errors.name} aria-describedby={errors.name?"c-name-err":undefined} value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder="Jane Doe" />
                      {errors.name && <div id="c-name-err" className="err-msg" role="alert">{errors.name}</div>}
                    </div>
                    <div className={`field ${errors.email ? "err" : ""}`}>
                      <label htmlFor="c-email">Email *</label>
                      <input id="c-email" name="email" type="email" autoComplete="email" aria-invalid={!!errors.email} aria-describedby={errors.email?"c-email-err":undefined} value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} placeholder="jane@example.com" />
                      {errors.email && <div id="c-email-err" className="err-msg" role="alert">{errors.email}</div>}
                    </div>
                  </div>
                  <div className="field-row">
                    <div className={`field ${errors.phone ? "err" : ""}`}>
                      <label htmlFor="c-phone">Phone</label>
                      <input id="c-phone" name="phone" type="tel" autoComplete="tel" aria-invalid={!!errors.phone} value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} placeholder="(515) 555-0199" />
                      {errors.phone && <div className="err-msg" role="alert">{errors.phone}</div>}
                    </div>
                    <div className={`field ${errors.service ? "err" : ""}`}>
                      <label htmlFor="c-service">Service *</label>
                      <select id="c-service" name="service" aria-invalid={!!errors.service} value={form.service} onChange={(e) => setForm({...form, service: e.target.value})}>
                  ...
                  <div className={`field ${errors.message ? "err" : ""}`}>
                    <label htmlFor="c-msg">Project details *</label>
                    <textarea id="c-msg" name="message" aria-invalid={!!errors.message} rows={5} value={form.message} onChange={(e) => setForm({...form, message: e.target.value})} placeholder="Briefly describe the project, location, and timeline." />
                    {errors.message && <div className="err-msg" role="alert">{errors.message}</div>}
                  </div>
